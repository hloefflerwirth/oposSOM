...
pipeline.2ndLvlSimilarityAnalysis <- function()
{
  ...
  spot.metagenes <- unique(unlist(sapply(get(paste("spot.list.",preferences$standard.spot.modules,sep=""))$spots, function(x) { x$metagenes })))
  ...
  for (i in seq_along(metagene.filter.list))
  {
    ...
    heatmap.wrap(x=s, col=color.palette.heatmaps(1000), main=paste("Clustering heatmap,",metagene.filter.list[[i]]$n),
