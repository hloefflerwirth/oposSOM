pipeline.3rdLvlChromosomeAssociationMaps <- function(gene.positions.list)
{
  all.chromos <- sort.label(names(gene.positions.list))
  all.chromos <- all.chromos[which(nchar(all.chromos) < 4)]

  all.chromos.pq <- paste(unlist(sapply(all.chromos, function(x)
  {
    rep(x, length(gene.positions.list[[x]]))
  })), unlist(sapply(gene.positions.list[all.chromos],names)))
  ...
  spot.gene.chromo.pq.matrix <-
    rbind(spot.gene.chromo.pq.matrix,
          colSums(spot.gene.chromo.pq.matrix) /
          max(colSums(spot.gene.chromo.pq.matrix)) *
          sqrt(max.enrichment))

  rownames(spot.gene.chromo.pq.matrix)[nrow(spot.gene.chromo.pq.matrix)] <- "summary"

  spot.gene.chromo.pq.matrix.scaled <-
    rbind(spot.gene.chromo.pq.matrix.scaled,
          colSums(spot.gene.chromo.pq.matrix.scaled) /
          max(colSums(spot.gene.chromo.pq.matrix.scaled)) *
          sqrt(max.enrichment.scaled))
  ...
  for (spot.i in 1:nrow(spot.gene.chromo.pq.matrix))
  {
    ...
    if (spot.i <= length(set.list$spots))
    {
      ...
      text(0.01, 0.7,
           paste("# genes in spot =",
                 length(set.list$spots[[spot.i]]$genes), " (",
                 sum(spot.gene.chromo.pq.matrix[spot.i,]),"with loci information)"), adj=0)

      text(0.01, 0.58,
           paste("# samples with spot =",
                 sum(samples.with.spot), "(",
                 round(100 * sum(samples.with.spot)/ncol(indata), 1), "%)"), adj=0)
        ...
        for (g in 1:length(samples.with.spot))
        {
          text(0.02, 0.58-g*0.1,
               paste(names(samples.with.spot)[g], ":",
                     samples.with.spot[g], "(",
                     round(100 * samples.with.spot[g]/sum(group.labels == names(samples.with.spot)[g]), 1), "%)"),
               adj=0,  col=group.colors[match(names(samples.with.spot)[g], group.labels)])
        }
      }
    }
    ...
    for (chr.i in 1:ncol(chromo.data))
    {
      for (chr.j in 1:ncol(chromo.data))
      {
        ...
        points(chr.i, chr.j, pch=16,
               cex=0 + 3 * n1 * n2 / max(chromo.data[spot.i,]^2), col="blue")
      }
    }
