pipeline.affymetrixQualityCheck <- function()
{
  ...
  {  
    ...
    for( i in spike.in.controls )
    {
      ...
      barplot( indata.original[i,], col=group.colors, main=paste("Intensity of sample preparation spike-in control",i), names.arg=if(ncol(indata)<80) colnames(indata) else rep("",ncol(indata)), las=2, cex.main=1, cex.lab=1, cex.axis=0.8, cex.names=0.6, border = ifelse(ncol(indata)<80,"black",NA), ylim=range( indata.original[i,] )*c(0.99,1.01), xpd=F )	
      box()
  ...
  {  
    ...
    barplot( ratio.3.5.GAPDH, col=group.colors, main="3' / 5' ratio GAPDH", names.arg=if(ncol(indata)<80) colnames(indata) else rep("",ncol(indata)), las=2, cex.main=1, cex.lab=1, cex.axis=0.8, cex.names=0.6, border = ifelse(ncol(indata)<80,"black",NA), ylim=c( min(ratio.3.5.GAPDH )*0.99 , max(1.26,max(ratio.3.5.GAPDH )*1.01) ), xpd=F )	
  ...
  {	
    ...
    barplot( ratio.3.5.betaact, col=group.colors, main="3' / 5' ratio beta-actin", names.arg=if(ncol(indata)<80) colnames(indata) else rep("",ncol(indata)), las=2, cex.main=1, cex.lab=1, cex.axis=0.8, cex.names=0.6, border = ifelse(ncol(indata)<80,"black",NA), ylim=c( min(ratio.3.5.betaact )*0.99 , max(3.01,max(ratio.3.5.betaact )*1.01) ), xpd=F )	
  ...
  {  
    ...
    plot( 0, type="n", xlim=c(1,4), ylim=range(c(indata.original["AFFX-BioB-3_at",],
                                                 ...
                                                 indata.original["AFFX-CreX-3_at",] )), xlab="", ylab="", axes=F, main="Intensities of spike-in controls" )
