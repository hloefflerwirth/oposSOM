...
for( gr in 1:length(unique(group.labels)))
{
	...
	{
		wilcox.test( x[which(group.labels!=unique(group.labels)[gr])], x[which(group.labels==unique(group.labels)[gr])], alternative="less" )$p.value		
		#wilcox.test( x[which(group.labels==unique(group.labels)[gr])], alternative="greater" )$p.value		
	} )
	...
	pdf( paste( files.name, " - Results/Summary Sheets - Groups/Geneset Analysis/Specific GS ",make.names(unique(group.labels)[gr]),".pdf", sep="" ), 21/2.54, 29.7/2.54 )
	...
	for( i in 1:20 )
	{
		...
		barplot( samples.GSZ.scores[ names(top.gs)[i], ], beside=T, las=2, cex.names=1.2, col=group.colors, cex.main=1, ylim=ylim, border = if( ncol(indata) < 80 ) "black" else NA, names.arg=if( ncol(indata)<100 ) colnames(indata) else rep("",ncol(indata) ), cex.axis=1.4 )
			abline(h=0,lty=2)
		...
		title(sub=paste( "# features =", length(gs.def.list[[ names(top.gs)[i] ]]$Genes), ", max =", max(n.map,na.rm=T) ),line=0.5, cex.sub=1.4)
		box()
	...
	write.csv2( out, paste( files.name, " - Results/Summary Sheets - Groups/Geneset Analysis/Specific GS ",make.names(unique(group.labels)[gr]),".csv", sep="" ), row.names=F )
