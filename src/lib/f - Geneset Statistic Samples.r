...
flush.console()
	...
	if( preferences$geneset.analysis.exact )
	{
		...
		out.intervals = round( seq( 1, ncol(indata), length.out=25+1 ) )[-1]
	...
	for( m in 1:ncol( indata ) )
	{
		...
		for( spot.i in 1:length( GS.infos.samples[[ m ]]$spots ) )
		{
			...
			{
				GS.infos.samples[[ m ]]$spots[[spot.i]]$GSZ.score = GeneSet.GSZ( spot.gene.ids, all.gene.statistic, gs.def.list, sort=F, cluster=cl )
				GS.infos.samples[[ m ]]$spots[[spot.i]]$Fisher.p = GeneSet.Fisher( spot.gene.ids, unique.protein.ids, gs.def.list, sort=T, cluster=cl )
			} else
			{
			...
			if(preferences$geneset.analysis.exact)
			{
				GS.infos.samples[[ m ]]$spots[[spot.i]]$GSZ.p.value = 1 - null.culdensity( abs( GS.infos.samples[[ m ]]$spots[[spot.i]]$GSZ.score ) )
				names(GS.infos.samples[[ m ]]$spots[[spot.i]]$GSZ.p.value) = names(GS.infos.samples[[ m ]]$spots[[spot.i]]$GSZ.score)
			}
	...
	out.intervals = round( seq( 1, ncol(indata), length.out=1 + if(preferences$geneset.analysis.exact) 20 else 45 ) )[-1]
	cat( paste( rep("#",length( which( out.intervals == m) ) ), collapse="" ) );	flush.console()
