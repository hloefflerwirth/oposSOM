...
#			thresh.global = max( max(set.list$spotdata), -min(set.list$spotdata) ) / 6
			...
			for( s in names(set.list$spots) )
			{
				if( main %in% c("Sample-Underexpression") )
					sample.spot.matrix[s,] = as.numeric( set.list$spotdata[s,] < mean(set.list$spotdata[s,])-thresh.global )
				else
					sample.spot.matrix[s,] = as.numeric( set.list$spotdata[s,] > mean(set.list$spotdata[s,])+thresh.global )
			}
			...
			pattern.adj.matrix = matrix( NA, ncol(pattern.spot.matrix), ncol(pattern.spot.matrix) )
		...
		#			if( length( common.pattern ) > 0 )
			...
			source("Functions/f - 3rd lvl Networks WTO.r",local=T)
			...
			for( i in 1:length(set.list$spots)  )
			{
				...
				mean.spot.expression = cbind(  mean.spot.expression, if( length(spot.metagenes) > 1 ) colMeans( metadata[ spot.metagenes, ] ) else metadata[ spot.metagenes, ] )
			}
			...
			if( max(adj.matrix) > 0 )
			{
				...
				plot( g, layout=layout, vertex.size=0, vertex.label="", xlim=c(-1.1,1.1), ylim=c(-1.1,1.1) )
				...
				for( i in 1:N.spots )
				{
					...
					rect( layout[i,1] - w, layout[i,2] - w, layout[i,1] + w, layout[i,2] + w, col="white" )
					x <- pixmapIndexed( mask , col = c( "white", "green" ) )
					addlogo( x, layout[i,1] + c(-w,w), layout[i,2] + c(-w,w) )
			
					rect( layout[i,1] - w, layout[i,2] - w, layout[i,1] + w, layout[i,2] + w )
		
		
		
					text( layout[i,1]-w+0.01, layout[i,2]+w+0.025, paste( "Total :", length( samples ) ), adj=0, cex=0.6 )
		
					for( ii in 1:length( tbl ) )
					{
						text( layout[i,1]-w+0.01, layout[i,2]+w+0.01  - ii*0.036, paste( names(tbl)[ii], ":", t2[ii], " - ", tbl[ii], "%" ), adj=0, cex=0.6, col=group.colors[ which( group.labels == names(tbl)[ii] )[1] ] )
					}
			...
			if( length( unique(group.labels) ) > 1 )
			{
				...
				for( i in 1:N.spots )
				{
					...
					tbl[ i, names(table( as.vector(group.labels[samples]) )) ] = table( as.vector(group.labels[samples]) )
				}
				tbl = tbl / max( tbl )
		
				stars( tbl, locations=layout, xlim=c(-1.1,1.1), ylim=c(-1.1,1.1), len=0.1, draw.segments=T, col.segments=unique.group.colors, key.loc=c(1.3,-1.1), scale=F )
			...
			if( ncol(pattern.adj.matrix) > 2 )
			{
				...
				if( max(adj.matrix) > 0 )
				{
					...
					for( j in 1:ncol(pattern.spot.matrix) )
					{
						...
						rect( layout[j,1] - w, layout[j,2] - w, layout[j,1] + w, layout[j,2] + w )
						...
						text( layout[j,1]-w+0.01, layout[j,2]+w+0.025, paste( "Total :", length( samples ) ), adj=0, cex=0.6 )
		
						for( i in 1:length( tbl ) )
						{
							text( layout[j,1]-w+0.01, layout[j,2]+w+0.01  - i*0.036, paste( names(tbl)[i], ":", t2[i], " - ", tbl[i], "%" ), adj=0, cex=0.6, col=group.colors[ which( group.labels == names(tbl)[i] )[1] ] )
						}
	...
	n = paste( files.name, " - Results/3rd lvl Spot Analysis/Overexpression Networks", if(GS.infos.overexpression$filtered) " filtered", ".pdf", sep="" )
	...
	n = paste( files.name, " - Results/3rd lvl Spot Analysis/Underexpression Networks", if(GS.infos.underexpression$filtered) " filtered", ".pdf", sep="" )
	pdf( n, 29.7/2.54, 21/2.54 )
	...
	n = paste( files.name, " - Results/3rd lvl Spot Analysis/K-Means Cluster Networks", ".pdf", sep="" )
	pdf( n, 29.7/2.54, 21/2.54 )
	...
	n = paste( files.name, " - Results/3rd lvl Spot Analysis/Group Overexpression Networks", ".pdf", sep="" )
	pdf( n, 29.7/2.54, 21/2.54 )
